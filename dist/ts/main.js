...
function showLoading(message = "Searching for poetry...") {
    if (loadingOverlay) {
...
function toggleFavorite() {
    if (!currentPoem || !favoriteBtn)
        return;
    const isFavorited = favorites.some(fav => fav.title === currentPoem.title && fav.author === currentPoem.author);
    if (isFavorited) {
        favorites = favorites.filter(fav => !(fav.title === currentPoem.title && fav.author === currentPoem.author));
...
function generateGrid(e) {
    ...
    const titleInput = document.querySelector("#poemTitle");
...
function renderGrid(poems, title = "Search Results") {
    ...
    if (resultsCount)
        resultsCount.textContent = `${poems.length} poem${poems.length !== 1 ? 's' : ''} found`;
...
function showLineText(lines, title, author) {
    ...
    if (favoriteBtn) {
        const isFavorited = favorites.some(fav => fav.title === title && fav.author === author);
        if (isFavorited) {
            favoriteBtn.classList.add("favorited");
            favoriteBtn.querySelector(".heart-icon").textContent = "♥";
            favoriteBtn.setAttribute("aria-label", "Remove from favorites");
